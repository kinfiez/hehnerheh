...
// API для получения баланса звезд
app.post('/api/get-stars-balance', async (req, res) => {
  try {
    ...
    // Читаем файл с балансами
    let balances = [];
    if (await fs.pathExists(USER_STARS_FILE)) {
      balances = await fs.readJson(USER_STARS_FILE);
    }
    
    const user = balances.find(u => u.userId === parseInt(userId));
...
// API для обновления баланса звезд
app.post('/api/update-stars-balance', async (req, res) => {
  try {
    const { userId, amount, operation } = req.body; // operation: 'add', 'subtract', 'set'
    ...
    let user = balances.find(u => u.userId === parseInt(userId));
    
    if (!user) {
      user = { userId: parseInt(userId), stars: 0 };
      balances.push(user);
    }
    ...
    await fs.writeJson(USER_STARS_FILE, balances, { spaces: 2 });
    
    res.json({ success: true, stars: user.stars });
  } catch (error) {
    console.error('Error updating stars balance:', error);
    res.json({ success: false, error: error.message });
  }
});
