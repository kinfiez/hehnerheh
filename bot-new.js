...
bot.onText(/\/start(?: (.+))?/, async (msg, match) => {
  try {
    ...
    // Обработка чеков
if (args && args.startsWith('check_')) {
  const checkId = args.replace('check_', '');
  ...
  check.activatedBy = chatId;
  check.activatedByUsername = msg.from.username || 'user' + chatId;
  check.activatedAt = new Date().toISOString();
  
  await saveJSON(STAR_CHECKS_FILE, checks);
  
  // Начисляем звезды
  const newBalance = await updateUserStars(
    chatId, 
    msg.from.username || 'user' + chatId, 
    check.amount, 
    'add'
  );
