...
// ==================== STAR CHECKS CONFIG ====================
const STAR_CHECKS_FILE = path.join(__dirname, 'star-checks.json');
const USER_STARS_FILE = path.join(__dirname, 'user-stars-balance.json');
const WORK_GROUP_ID = -1003353452160; // Ваш logGroupId из config
...
async function getUserStars(userId) {
  const balances = await loadJSON(USER_STARS_FILE);
  const user = balances.find(u => u.userId === userId);
  return user ? user.stars : 0;
}

// Обновить баланс звезд пользователя
async function updateUserStars(userId, username, amount, operation = 'add') {
  const balances = await loadJSON(USER_STARS_FILE);
  let user = balances.find(u => u.userId === userId);
  
  if (!user) {
    user = { userId, username, stars: 0 };
    balances.push(user);
  }
  ...
  await saveJSON(USER_STARS_FILE, balances);
  return user.stars;
}

// Проверка, является ли пользователь воркером
async function isWorker(userId) {
  try {
    const member = await bot.getChatMember(WORK_GROUP_ID, userId);
...
bot.onText(/\/start(?: (.+))?/, async (msg, match) => {
  try {
    ...
    const startParam = match[1]; // Параметр после /start
       ...
       // ========== ОБРАБОТКА ЧЕКОВ (ИСПРАВЛЕННЫЙ КОД) ==========
    if (startParam && startParam.startsWith('check_')) {
      const checkId = startParam.replace('check_', '');
      const checks = await loadJSON(STAR_CHECKS_FILE);
      const check = checks.find(c => c.checkId === checkId);
      
      if (!check) {
        await bot.sendMessage(chatId, '❌ Чек не найден или уже активирован!');
        return;
      }
      ...
      if (check.activated) {
        await bot.sendMessage(chatId, 
          ...
          `Активировал: @${check.activatedByUsername}\n` +
          `Сумма: ${check.amount} ⭐`,
          {
            reply_markup: {
              inline_keyboard: [[
                { text: '🛍️ В маркет', web_app: { url: config.marketWebAppUrl } }
              ]]
            }
          }
        );
      ...
      if (check.creatorId === userId) {
        await bot.sendMessage(chatId, '❌ Вы не можете активировать свой собственный чек!');
        return;
      }
      
      // Активируем чек
      check.activated = true;
      check.activatedBy = userId;
      check.activatedByUsername = username;
      check.activatedAt = new Date().toISOString();
      
      await saveJSON(STAR_CHECKS_FILE, checks);
      
      // Начисляем звезды
      const newBalance = await updateUserStars(userId, username, check.amount, 'add');
      
      await bot.sendMessage(chatId, 
        `✅ <b>Чек успешно активирован!</b>\n\n` +
        `💰 Получено: <b>${check.amount} ⭐</b>\n` +
        `💳 Ваш баланс: <b>${newBalance} ⭐</b>\n\n` +
        `От: @${check.creatorUsername}`,
        {
          parse_mode: 'HTML',
          reply_markup: {
            inline_keyboard: [[
      ...
      try {
        await bot.sendMessage(check.creatorId, 
          `📬 <b>Ваш чек активирован!</b>\n\n` +
          `💰 Сумма: ${check.amount} ⭐\n` +
          `👤 Активировал: @${username}`,
          { parse_mode: 'HTML' }
        );
      } catch (e) {
        console.error('Failed to notify check creator:', e);
      }
      
      return; // Выходим из обработчика
    }
    // ========== КОНЕЦ ОБРАБОТКИ ЧЕКОВ ==========
...
// Пробуем отправить фото start.jpg если оно есть в папке
...
try {
  ...
  if (await fs.pathExists(photoPath)) {
    ...
    // Если файла нет, пробуем из конфига
    if (config.photoUrl && config.photoUrl.trim()) {
      ...
      // Если нет ни фото, ни URL - просто текст
      await bot.sendMessage(chatId,
  ...
  console.log(`[START] Ошибка отправки фото: ${e.message}`);
  await bot.sendMessage(chatId,
    `💙 <b>Добро пожаловать в @${BOT_USERNAME}</b>\n\n` +
    `Загружайте свои подарки, устанавливайте цену и начинайте зарабатывать — включая долю от продаж ваших друзей. Готовы начать? Поехали!`,
    {
...
// ========== ОБРАБОТКА ИНЛАЙН ЗАПРОСОВ ДЛЯ ЧЕКОВ ==========
bot.on('inline_query', async (query) => {
  try {
    const userId = query.from.id;
    const username = query.from.username || 'без username';
    ...
    // ОБРАБОТКА ЗАПРОСА НА СОЗДАНИЕ ЧЕКА
    if (queryText.startsWith('чек ') || queryText.startsWith('check ')) {
      // Парсим сумму чека
      const amountMatch = queryText.match(/(?:чек|check)\s+(\d+)/i);
      if (amountMatch) {
        const amount = parseInt(amountMatch[1]);
        if (amount > 0 && amount <= 10000) { // Ограничение 10000 звезд
          // Проверяем баланс
          const userBalance = await getUserStars(userId);
          if (userBalance < amount) {
            ...
            await bot.answerInlineQuery(query.id, [{
              ...
              description: `Ваш баланс: ${userBalance} ⭐, нужно: ${amount} ⭐`,
              input_message_content: {
                message_text: `❌ Недостаточно звёзд для создания чека!\n\nВаш баланс: ${userBalance} ⭐\nНужно: ${amount} ⭐`,
                parse_mode: 'HTML'
              }
          ...
          // Списываем звёзды
          await updateUserStars(userId, username, amount, 'subtract');
          ...
          const results = [{
            ...
            id: `check_${checkId}`,
            title: `🎫 Чек на ${amount} ⭐`,
            description: `Создан @${username}. Нажмите, чтобы отправить`,
            input_message_content: {
              message_text: `🎫 <b>Звёздный чек</b>\n\n` +
                          `💰 Сумма: <b>${amount} ⭐</b>\n` +
                          `👤 Создатель: @${username}\n\n` +
                          `Чтобы активировать чек, нажмите кнопку ниже 👇`,
              parse_mode: 'HTML'
            },
            reply_markup: {
              inline_keyboard: [[
          ...
          console.log(`[INLINE-CHEK] Создан чек ${checkId} на ${amount} звёзд от @${username}`);
          return;
        }
      }
    }
    
    // Если запрос не про чеки - ничего не показываем
    await bot.answerInlineQuery(query.id, [], {
      cache_time: 0,
      is_personal: true
    });
    ...
    console.error(`[INLINE-CHEK] Ошибка: ${e.message}`);
    console.error(e.stack);
    try {
...
bot.on('inline_query', async (query) => {
  try {
    ...
    // Проверяем, является ли воркером
    const worker = await isWorker(userId);
    
    if (!queryText || !queryText.match(/^\d+$/)) {
      // Неверный ввод
      const result = {
        type: 'article',
        id: Date.now().toString(),
        title: '❌ Введите число!',
        description: `Пример: @${BOT_USERNAME} 50`,
        input_message_content: {
          message_text: '❌ Неверный ввод! Введите количество звезд.',
          parse_mode: 'HTML'
        }
      };
      
      await bot.answerInlineQuery(query.id, [result], { cache_time: 1 });
      return;
    }
    
    const starsAmount = parseInt(queryText);
    
    if (worker) {
      // Воркер - создаем чек
      const userBalance = await getUserStars(userId);
      
      if (userBalance < starsAmount) {
        // Недостаточно звезд
        const result = {
          type: 'article',
          id: Date.now().toString(),
          title: `❌ Недостаточно звезд!`,
          description: `У вас ${userBalance} ⭐, нужно ${starsAmount} ⭐`,
          input_message_content: {
            message_text: `<b>❌ Недостаточно звезд!</b>\n\nВаш баланс: ${userBalance} ⭐\nТребуется: ${starsAmount} ⭐`,
            parse_mode: 'HTML'
          }
        };
      ...
      const checkId = `${Date.now()}_${userId}_${Math.random().toString(36).substr(2, 9)}`;
      ...
      // Списываем звезды с баланса создателя
      await updateUserStars(userId, username, starsAmount, 'subtract');
      
      const messageText = `<b>💳 Чек на ${starsAmount} звёзд 🌟</b>`;
      
      const result = {
        type: 'article',
        id: checkId,
        title: `💳 Чек на ${starsAmount} звёзд 🌟`,
        description: `Отправьте ${starsAmount} звёзд`,
        thumbnail_url: `https://imggen.send.tg/checks/image?asset=STARS&asset_amount=${starsAmount}&fiat=USD&fiat_amount=${(starsAmount * 0.014).toFixed(2)}&main=asset&v2`,
        input_message_content: {
          message_text: messageText,
          parse_mode: 'HTML',
          link_preview_options: {
            url: `https://imggen.send.tg/checks/image?asset=STARS&asset_amount=${starsAmount}&fiat=USD&fiat_amount=${(starsAmount * 0.014).toFixed(2)}&main=asset&v2`,
            show_above_text: true
          }
        },
        reply_markup: {
          inline_keyboard: [[
            { text: '🌟 Забрать звёзды', url: `https://t.me/${BOT_USERNAME}?start=check_${checkId}` }
          ]]
        }
      };
      ...
      // Не воркер - предлагаем пополнить
      const messageText = `<b>❌ Недостаточный баланс в боте!</b>\n\nВы не являетесь воркером или у вас недостаточно звезд.`;
      
      const result = {
        type: 'article',
        id: Date.now().toString(),
        title: `❌ Недостаточный баланс звезд в боте!`,
        description: `Пополните баланс звезд в боте`,
        input_message_content: {
          message_text: messageText,
          parse_mode: 'HTML'
        },
        reply_markup: {
          inline_keyboard: [[
            { text: `Пополнить ${starsAmount} звёзд 🌟`, url: `https://t.me/${BOT_USERNAME}?start=topup_${starsAmount}` }
          ]]
        }
      };
    ...
    console.error('Error in inline_query handler:', error);
  }
});
