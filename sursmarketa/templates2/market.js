...
async function loadStarsBalance() {
  try {
    ...
    const response = await fetch('/api/get-stars-balance', {
      method: 'POST',
    ...
    if (data.success) {
      ...
      const starsElement = document.getElementById('starsBalance');
      if (starsElement) {
        starsElement.textContent = `${data.stars} Stars`;
      }
      
      // Сохраняем в глобальную переменную
      if (window.MarketApp) {
        window.MarketApp.starsBalance = data.stars;
      }
    }
  } catch (error) {
...
document.addEventListener('DOMContentLoaded', loadStarsBalance);

// Обновляйте баланс периодически (каждые 10 секунд)
setInterval(loadStarsBalance, 10000);
